...
extern uint8_t Ov7725_vsync;	 /* 帧同步信号标志，在中断函数和main函数里面使用 */
...
#define FIFO_CS_H()     GPIO_SetBits(GPIOA,GPIO_Pin_3)	  
#define FIFO_CS_L()     GPIO_ResetBits(GPIOA,GPIO_Pin_3)	  /*拉低使FIFO输出使能*/

#define FIFO_WRST_H()   GPIO_SetBits(GPIOB,GPIO_Pin_5)		  /*拉高允许FIFO写(数据from摄像头)指针运动 */
#define FIFO_WRST_L()   GPIO_ResetBits(GPIOB,GPIO_Pin_5)	  /*拉低使FIFO写(数据from摄像头)指针复位*/

#define FIFO_RRST_H()   GPIO_SetBits(GPIOA,GPIO_Pin_2)	    /*拉高允许FIFO读(数据从FIFO输出)指针运动 */
#define FIFO_RRST_L()   GPIO_ResetBits(GPIOA,GPIO_Pin_2)	  /*拉低使FIFO读(数据从FIFO输出)指针复位 */

#define FIFO_RCLK_H()   GPIO_SetBits(GPIOC,GPIO_Pin_5)
#define FIFO_RCLK_L()   GPIO_ResetBits(GPIOC,GPIO_Pin_5)    /*FIFO输出数据时钟*/

#define FIFO_WE_H()     GPIO_SetBits(GPIOD,GPIO_Pin_3)      /*拉高使FIFO写允许*/
#define FIFO_WE_L()     GPIO_SetBits(GPIOD,GPIO_Pin_3)

#define READ_FIFO_PIXEL(RGB565)   	do{\
	                                  RGB565=0;\
	                                  FIFO_RCLK_L();\
	                                  RGB565 = (GPIOC->IDR) & 0xff00;\
	                                  FIFO_RCLK_H();\
                                    FIFO_RCLK_L();\
	                                  RGB565 |= (GPIOC->IDR >>8) & 0x00ff;\
	                                  FIFO_RCLK_H();\
                                    }while(0)
...
void ov7725_InitConfig(void);			//ov7725总的初始化函数
...
/*
*********************|			驱动过程			|****************************************
			1.先将端口初始化  Ov7725_GPIO_Config();
			2.ov7725 寄存器配置初始化 while(Ov7725_Init() != SUCCESS);
			3.ov7725 场信号线初始化   	VSYNC_Init();			Ov7725_vsync = 0;
			4.接收显示数据					
			if( Ov7725_vsync == 2 )
			{
				FIFO_PREPARE;  					
...
?************************************************************************************
				  ...
				  2. FIFO数据传输控制引脚
					...
					3. PA0---VSYNC   场信号线    
							HREF行中断 和WEN通过与非门控制WE   所以不用分配引脚
							
			外部移植时需要引用  此中断信号标志   uint8_t Ov7725_vsync = 0;	
*/
